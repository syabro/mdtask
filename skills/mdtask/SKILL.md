...
> These examples focus on the spec/journal rhythm — how tasks and the feature description evolve together as work is documented. For the full task-body shape (value summary, prose, `DoD`), see the `## Task body` section below; the kettle snippets are deliberately minimal.
