...
mdtask ships its dev skills (`mdtask`, `mdtask-add`, `mdtask-do`) inside the npm package. A skill auto-invokes only when its `SKILL.md` lives in the agent's own skill-discovery folder, which differs per agent — so the agent passes that folder in:
